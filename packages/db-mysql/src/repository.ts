 ...
 * ```
 */
export class MySQLRepository<T extends Record<string, unknown>, PK = number> implements IRepository<T, PK> {
  constructor(
  ...
  query(): IQueryBuilder<T> {
    return new MySQLQueryBuilder<T>(this.getPool).from(this.table);
  }
  ...
  private async rawQuery<R extends Record<string, unknown> = Record<string, unknown>>(
    sql: string,
    params: unknown[] = [],
    ...
    return {
      rows: rowArray,
      rowCount: (rows as { affectedRows?: number }).affectedRows ?? rowArray.length,
      raw: rows,
    };
  }
}
